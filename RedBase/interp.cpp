...
static void print_op(CompOp comparisonOp);
...
RC interp(NODE *n)
{
	...
	case N_CREATETABLE:            /* for CreateTable() */
	{
		...
		nattrs = mk_attr_infos(n->u.CREATETABLE.attrlist, MAXATTRS, attrInfos);
		if (nattrs < 0) {
	...
	case N_INSERT:            /* for Insert() */
	{
		...
		errval = pQlm->Insert(n->u.INSERT.relname, nValues, values);
		break;
	}
	...
	if (errval == 0) {
		printf("Command executed with succesful!\n");
	}
...
static int mk_conditions(NODE *list, int max, Condition conditions[])
{
	...
	for (i = 0; list != NULL; ++i, list = list->u.LIST.next) {
		...
		conditions[i].comparisonOp = current->u.CONDITION.comparisonOp;
...
static int parse_format_string(char *format_string, AttrType *type, int *len)
{
	...
	n = sscanf_s(format_string, "%c%d", &c, sizeof(c), len);
...
static void print_condition(NODE *n)
{
	...
	print_op(n->u.CONDITION.comparisonOp);
