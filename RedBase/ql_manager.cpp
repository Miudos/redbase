...
#pragma region Insert
	RC QL_Manager::Insert(const char *relName,
		...
		for (unsigned int i = 0; i < attributes.size(); i++) {
			...
			sprintf_s(fname, sizeof(fname), "%s.%s.sorted", attributes[i].relName, attributes[i].attrName);
			if (_access(fname, 0) == 0) _unlink(fname);
		}
		///////////////////////////////////////////////////////////
...
#pragma region Delete
	RC QL_Manager::Delete(const char *relName,
		...
		if (nConditions == 0) {
			if (bQueryPlans) {
				cout<<"-- Indexes and relations will be dropped and recreated"<<endl;
			}
		...
		if (idxno < 0) {
			...
			for (int i = 0; i < nConditions; i++) {
				if (!conditions[i].bRhsIsAttr) {
					found = true;
					attrInd = findAttr(0, conditions[i].lhsAttr.attrName, attributes);
					value = conditions[i].rhsValue.data;
					cmp = conditions[i].comparisonOp;
					break;
				}
			}
			...
			scanner.reset(new QL_FileScan(rmm, ixm, relName, attrInd, cmp, value, NO_HINT, attributes));
			if (bQueryPlans) {
			...
			// use index scan
			CompOp cmp = conditions[idxno].comparisonOp;
			int attrInd = findAttr(0, conditions[idxno].lhsAttr.attrName, attributes);
	...
	RC QL_Manager::Update(const char *relName,
		...
		for (int i = 0; i < nConditions; i++) {
			...
			if (strcmp(conditions[i].lhsAttr.attrName, updAttr.attrName) == 0)
				continue;
			if (conditions[i].comparisonOp != EQ_OP) continue;
		...
		if (indexCond < 0) {
			...
			CompOp cmp = conditions[indexCond].comparisonOp;
			scanner.reset(new QL_IndexScan(rmm, ixm, relName, attrIndex, cmp, conditions[indexCond].rhsValue.data, NO_HINT, attributes));
			if (bQueryPlans) {
				cout << "INDEX SCAN ON " <<
		...
		if (attributes[upInd].indexNo >= 0) {
			QL_ErrorForward(ixm->OpenIndex(relName, attributes[upInd].indexNo, update_indh));
		}
		RID rid;
		vector<char> data;
		bool isValid = false;
		...
		while (scanner->Next(data, rid) == OK_RC) {
			...
			if (isValid) {
				...
				updatedRec.rid = rid;
				updatedRec.record = new char[attributes.back().offset + attributes.back().attrLength];
				memcpy(updatedRec.record, &data[0], attributes.back().offset + attributes.back().attrLength);
				updatedRec.bIsAllocated = 1;

				void *updatePos = (void*)(updatedRec.record + attributes[upInd].offset);
				if (bIsValue) {
				...
				if (attributes[upInd].indexNo >= 0) {
					QL_ErrorForward(update_indh.DeleteEntry((void*) &data[attributes[upInd].offset], rid));
...
#pragma region Private_Methods
	#define QL_operator(expr) do { \
		char *attr_value1 = new char[len1+1];\
		buffer(attr1, attr_value1, len1);\
		char *attr_value2 = new char[len2+1];\
...
#pragma region General_Methods
	...
	bool QL_Manager::evalCondition(void* data, const Condition &cond,
		const vector<DataAttrInfo> &attributes) {
		bool(*comp)(void* attr1, void* attr2, int len1, int len2, AttrType type);
