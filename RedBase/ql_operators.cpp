#pragma region Includes
	#include <sstream> //required to compile successfully ql_internal.h
	//#include <memory> //required to compile successfully ql_internal.h
...
void QL_Optimizer::pushCondition(QL_Op* &root) {
	...
	if (cond->child->opType == COND) {
		...
		QL_Op* iscan = new QL_IndexScan(down->rmm, down->ixm,
			down->relName.c_str(), attrIndex, cond->cond->comparisonOp,
...
bool QL_Optimizer::compatibleProjCond(QL_Projection* proj, QL_Condition* cond, QL_Op* &newproj) {
