...
RC EX_Loader::Create(char *fileName, float ff, int recsize,
	...
	sprintf_s(indFile, "%s.idx", fileName);
	return OK_RC;
}
...
RC EX_Sorter::sort(const char *fileName, float ff, bool makeIndex) {
	...
	while (fileq.size() > 1) {
		...
		for (int i = 0; i < min(qsize, buffSize); i++) {
			...
			sprintf_s(temp, "_%s.%d", fileName, fileq.front());
		...
		sprintf_s(temp, "_%s.%d", fileName, chunkNum);
		fileq.push(chunkNum);
		chunkNum++;
		...
		for (unsigned int i = 0; i < scanners.size(); i++) {
			...
			delete scanners[i];
			char temp[3 * MAXNAME];
			sprintf_s(temp, "_%s.%d", fileName, chunkIndices[i]);
			if (_unlink(temp) != 0) return WARN;
		}
		chunkIndices.clear();
...
RC EX_Sorter::createSortedChunk(const char *fileName, int chunkNum, float ff,
	...
	printf(&fname[0], "_%s.%d", fileName, chunkNum);
...
EX_Sort::EX_Sort(PF_Manager* pfm, QL_Op &child, int attrIndex) {
	...
	if (deleteAtClose) {
		...
		printf_s(fileName, "%s.%s.sorted", attributes[attrIndex].relName, attributes[attrIndex].attrName);
	}
	this->attrIndex = attrIndex;
...
RC EX_Sort::Open() {
	RC WARN = 511, ERR = -511;
	RC rc = OK_RC;
	if (deleteAtClose || _access(fileName, 0) != 0) {
...
bool EX_Optimizer::okToSort(QL_Op* root) {
	...
	if (root->opType == COND) {
		auto condOp = (QL_Condition*)root;
		CompOp cmp = condOp->cond->comparisonOp;
